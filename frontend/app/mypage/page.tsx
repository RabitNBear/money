          ...
          <div className="lg:col-span-9">
            {activeTab === 'portfolio' && (
              <div className="space-y-10 sm:space-y-12 animate-fade-in">
                ...
                {/* 총 투자 요약 박스 (4종으로 확대 및 반응형) */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                  <div className="p-5 sm:p-6 bg-[#f3f4f6] rounded-3xl">
                    <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest pl-1">Total Invested</p>
                    <p className="text-[15px] sm:text-[20px] font-black">{formatCurrency(portfolioSummary.totalInvested)}</p>
                  </div>
                  <div className="p-5 sm:p-6 bg-[#f3f4f6] rounded-3xl">
                    <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest pl-1">Final Market Value</p>
                    <p className="text-[15px] sm:text-[20px] font-black text-black">{formatCurrency(portfolioSummary.totalMarketValue)}</p>
                  </div>
                  <div className="p-5 sm:p-6 bg-[#f3f4f6] rounded-3xl">
                    <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest pl-1">Total P/L Amount</p>
                    <p className={`text-[15px] sm:text-[20px] font-black ${portfolioSummary.totalPL >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
                      {portfolioSummary.totalPL >= 0 ? '+' : ''}{formatCurrency(portfolioSummary.totalPL)}
                    </p>
                  </div>
                  <div className="p-5 sm:p-6 bg-[#f3f4f6] rounded-3xl">
                    <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest pl-1">Total P/L Ratio</p>
                    <p className={`text-[15px] sm:text-[20px] font-black ${portfolioSummary.totalPL >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
                  ...
                  {paginatedPortfolio.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="p-5 sm:p-8 border border-gray-100 rounded-[24px] bg-white shadow-sm transition-all relative">
                        <button onClick={() => deletePortfolioItem(item.id)} className="absolute top-5 sm:top-8 right-5 sm:right-8 text-[10px] font-black text-gray-300 hover:text-red-500 uppercase tracking-widest cursor-pointer transition-all">Delete</button>
                        ...
                        <div className="grid grid-cols-3 gap-x-2 sm:gap-x-12 gap-y-6 sm:gap-y-8">
                          <div><p className="text-[8px] sm:text-[10px] font-black text-gray-300 uppercase mb-1">손익금액</p><p className={`text-[12px] sm:text-[19px] font-black ${isProfit ? 'text-red-500' : 'text-blue-500'}`}>{isProfit ? '+' : ''}{formatCurrency(profitLoss)}</p></div>
                          <div><p className="text-[8px] sm:text-[10px] font-black text-gray-300 uppercase mb-1">나의 총 금액</p><p className="text-[12px] sm:text-[19px] font-black text-gray-900">{formatCurrency(totalInvested)}</p></div>
                          <div className="text-right"><p className="text-[8px] sm:text-[10px] font-black text-gray-300 uppercase mb-1">나의 금액</p><p className="text-[12px] sm:text-[19px] font-black text-gray-900">{formatCurrency(item.avgPrice)}</p></div>
                          <div><p className="text-[8px] sm:text-[10px] font-black text-gray-300 uppercase mb-1">손익백분율</p><p className={`text-[12px] sm:text-[19px] font-black ${isProfit ? 'text-red-500' : 'text-blue-500'}`}>{isProfit ? '+' : ''}{item.change}%</p></div>
                          <div><p className="text-[8px] sm:text-[10px] font-black text-gray-300 uppercase mb-1">시장 총 금액</p><p className="text-[12px] sm:text-[19px] font-black text-gray-900">{formatCurrency(totalMarketValue)}</p></div>
                          <div className="text-right"><p className="text-[8px] sm:text-[10px] font-black text-gray-300 uppercase mb-1">시장 금액</p><p className="text-[12px] sm:text-[19px] font-black text-gray-900">{formatCurrency(item.currentPrice)}</p></div>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="space-y-10 sm:space-y-12 animate-fade-in">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b-2 border-black pb-6 gap-4">
                  ...
                  <button
                    ...
                    className="bg-black text-white px-5 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-tighter hover:bg-gray-800 transition-all cursor-pointer shadow-lg active:scale-95"
                  >
                    + Add Schedule
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  <div className="lg:col-span-8 space-y-6">
                    <div className="flex justify-between items-center px-1">
                      <div className="flex items-center gap-3">
                        <span className="text-[18px] font-black uppercase tracking-tighter">{monthLabel}</span>
                    ...
                    <div className="grid grid-cols-7 border-t border-l border-gray-100 rounded-3xl overflow-hidden shadow-sm bg-white">
                      {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(d => (
                        <div key={d} className="text-center text-[10px] font-black text-gray-300 py-3 bg-[#fafafa] border-r border-b border-gray-100 uppercase">{d}</div>
                      ))}
                      {calendarDays.map((day, idx) => {
                        ...
                        return (
                          <div
                            ...
                            className={`min-h-[85px] sm:min-h-[110px] p-1.5 flex flex-col border-r border-b border-gray-100 cursor-pointer transition-all relative group ${isCurrMonth ? 'bg-white' : 'bg-[#fafafa] opacity-30 pointer-events-none'} ${isSelected ? 'bg-zinc-50' : 'hover:bg-gray-50'}`}
                          >
                            <span className={`text-[11px] sm:text-[13px] font-black mb-1 flex items-center justify-center w-6 h-6 rounded-full ${isSelected ? 'bg-black text-white shadow-md' : 'text-gray-300 group-hover:text-black'}`}>{day.getDate()}</span>
                            <div className="space-y-0.5 mt-0.5 overflow-hidden">
                              {dayMyScheds.slice(0, 1).map(s => (<div key={s.id} className="px-1 py-0.5 bg-blue-500 rounded text-[8px] font-bold text-white truncate">{s.title}</div>))}
                              {dayEcoEvents.slice(0, 1).map(e => (<div key={e.id} className="px-1 py-0.5 bg-zinc-800 rounded text-[8px] font-bold text-white truncate flex items-center gap-1"><span>{e.country === 'KR' ? '🇰🇷' : '🇺🇸'}</span><span className="truncate">{e.event}</span></div>))}
                  ...
                  <div className="lg:col-span-4 space-y-6">
                    <div className="p-6 bg-black text-white rounded-[24px] shadow-2xl animate-in fade-in zoom-in-95 duration-300">
                      <p className="text-[10px] font-black text-white/40 uppercase mb-1 tracking-widest">{format(selectedCalendarDay, 'EEEE')}</p>
                      <p className="text-[20px] font-black tracking-tighter">{format(selectedCalendarDay, 'MMM dd, yyyy')}</p>
                    </div>
                    ...
                    <div className="space-y-3 max-h-[450px] lg:max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                      {mySchedules.filter(s => s.date === format(selectedCalendarDay, 'yyyy-MM-dd')).map(s => (
                        <div key={s.id} className="p-5 bg-blue-50 border border-blue-100 rounded-2xl shadow-sm group flex flex-col justify-between min-h-[100px] animate-in slide-in-from-right-2">
                          <p className="text-[14px] font-black text-blue-900 leading-tight">{s.title}</p>
                          <div className="flex gap-3 self-end mt-2">
                            <button onClick={() => openEditModal(s)} className="text-[10px] font-black text-blue-400 hover:text-blue-600 uppercase transition-colors">Edit</button>
                            <button onClick={() => deleteMySchedule(s.id)} className="text-[10px] font-black text-blue-400 hover:text-red-500 uppercase transition-colors">Del</button>
                          </div>
                        </div>
                      ))}
                      {economicEvents.filter(e => e.date === format(selectedCalendarDay, 'yyyy-MM-dd')).map(e => (
                        <div key={e.id} className="p-5 bg-white border border-gray-100 rounded-2xl shadow-sm space-y-4 animate-in slide-in-from-right-2">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-2"><span className="text-xl">{e.country === 'KR' ? '🇰🇷' : '🇺🇸'}</span><span className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">{e.country} Market</span></div>
                          ...
                          <p className="text-[14px] font-black text-gray-900 leading-tight">{e.event}</p>
                          <div className="inline-block px-3 py-1 bg-[#f3f4f6] rounded-full text-[9px] font-black text-gray-400 uppercase tracking-widest">{e.importance} Impact</div>
                        </div>
                      ))}
                      {mySchedules.filter(s => s.date === format(selectedCalendarDay, 'yyyy-MM-dd')).length === 0 && economicEvents.filter(e => e.date === format(selectedCalendarDay, 'yyyy-MM-dd')).length === 0 && (
                        <div className="py-20 text-center border-2 border-dashed border-gray-100 rounded-[24px]"><Globe size={24} className="mx-auto mb-3 text-gray-100" /><p className="text-[12px] font-bold text-gray-300 italic uppercase">No schedules</p></div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  {paginatedWatchlist.map((item) => (
                    <div key={item.id} className="border border-gray-100 rounded-2xl p-6 sm:p-8 space-y-6 sm:space-y-8 bg-white shadow-sm flex flex-col justify-between">
                      <div className="flex justify-between items-start">
                        <div><h5 className="text-[17px] sm:text-[18px] font-black tracking-tighter">{item.name}</h5><p className="text-[10px] sm:text-[11px] font-bold text-gray-300 uppercase tracking-widest">{item.ticker}</p></div>
                        <span className={`text-[13px] sm:text-[14px] font-black ${item.dayChange >= 0 ? 'text-red-500' : 'text-blue-500'}`}>{item.dayChange >= 0 ? '▲' : '▼'} {Math.abs(item.dayChange)}%</span>
                      </div>
                  ...
                  {paginatedInquiries.map((item) => (
                    <div key={item.id} className="border border-gray-100 rounded-2xl overflow-hidden bg-white p-6 sm:p-8 space-y-6">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-4 sm:gap-6"><button onClick={() => deleteInquiryItem(item.id)} className="text-[10px] font-black text-gray-400 hover:text-red-500 uppercase tracking-widest cursor-pointer">Delete</button><span className={`text-[10px] sm:text-[11px] font-black uppercase tracking-widest ${item.status === '답변완료' ? 'text-blue-500' : 'text-gray-300'}`}>{item.status}</span><span className="text-[13px] sm:text-[14px] font-bold text-gray-300 italic tracking-tighter">{item.date}</span></div>
                      </div>
                      <div className="bg-[#f9fafb] p-6 sm:p-8 rounded-xl border border-gray-50"><div className="flex gap-4 sm:gap-5"><span className="text-[18px] sm:text-[20px] font-black text-blue-500">A.</span><p className="text-[14px] sm:text-[15px] leading-relaxed text-gray-600 font-medium">{item.answer}</p></div></div>
                    </div>
                  ))}
                </div>
                  ...
                  <section className="pt-10 border-t border-gray-100 space-y-6">
                    ...
                    <div className="p-6 sm:p-8 border border-red-100 rounded-3xl bg-red-50/30 space-y-8">
                      <div className="text-center sm:text-left">
                        <p className="text-[16px] font-black text-gray-900">회원 탈퇴</p>
          ...
          <div className="relative bg-white w-full max-w-[450px] rounded-[32px] p-8 sm:p-10 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleAddMySchedule} className="space-y-8">
              <div className="space-y-2">
                ...
                <div className="w-full h-[60px] bg-[#f3f4f6] rounded-2xl px-6 flex items-center font-black text-[15px] text-gray-900">{format(selectedCalendarDay, 'yyyy-MM-dd')}</div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Schedule Title</label>
                <input autoFocus type="text" placeholder="일정 내용을 입력하세요" value={newSchedTitle} onChange={(e) => setNewSchedTitle(e.target.value)} className="w-full h-[64px] bg-[#f3f4f6] rounded-2xl px-6 font-black text-[16px] outline-none focus:ring-1 focus:ring-black transition-all" />
              </div>
              <div className="pt-4"><button type="submit" className="w-full h-[72px] bg-black text-white rounded-2xl font-black text-[16px] uppercase tracking-widest hover:bg-gray-800 transition-all shadow-xl cursor-pointer">{editingId ? 'Update Schedule' : 'Add to Calendar'}</button></div>
            </form>
          </div>
        </div>
      )}
...
function Pagination({ currentPage, totalPages, onPageChange }: PaginationProps) {
  ...
  return (
    <div className="flex justify-center items-center gap-2 pt-10">
      <button onClick={() => handlePageClick(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="w-10 h-10 flex items-center justify-center rounded-full text-black hover:bg-gray-100 cursor-pointer disabled:opacity-20 transition-all">〈</button>
      {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
        <button key={num} onClick={() => handlePageClick(num)} className={`w-10 h-10 rounded-full text-[13px] font-black transition-all cursor-pointer ${num === currentPage ? 'bg-black text-white shadow-lg' : 'text-black hover:bg-gray-50'}`}>{num}</button>
      ))}
      <button onClick={() => handlePageClick(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="w-10 h-10 flex items-center justify-center rounded-full text-black hover:bg-gray-100 cursor-pointer disabled:opacity-20 transition-all">〉</button>
    </div>
  );
}
